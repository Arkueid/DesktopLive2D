...
class GalDialog:
    ...
    def __show(self, text, x, y, duration):
        if self.dialog:
            self.dialog.destroy()
    ...
    def trigger_from_thread(self, text, x, y, duration=2):
        self.root.after(0, lambda: self.__show(text, x, y, duration))

    def move_from_thread(self, x, y):
        self.root.after(0, lambda: self.__doMove(x, y))
